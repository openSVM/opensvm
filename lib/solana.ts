...
export interface ParsedInstructionWithAccounts
  extends Omit<ParsedInstruction, "accounts"> {
  accounts: number[];
  data: string;
}

export interface PartiallyDecodedInstructionWithAccounts
  extends Omit<PartiallyDecodedInstruction, "accounts"> {
  accounts: number[];
  data: string;
}

export type InstructionWithAccounts =
  | ParsedInstructionWithAccounts
  | PartiallyDecodedInstructionWithAccounts;
...
export async function getTokenAccountsCustom(
  address: string,
): Promise<AccountData> {
  const connection = await getProxyConnection();
  ...
  try {
    ...
    const metadataPromises = tokenAccountsResponse.value.map((ta) =>
      getTokenMetadataFromRPC(ta.account.data.parsed.info.mint).catch(() => ({
        symbol: "",
        name: "",
      })),
    );
  ...
  const isSystemProgram = accountInfo.owner.equals(
    new PublicKey("11111111111111111111111111111111"),
  );
  const isTokenProgram = accountInfo.owner.equals(
...
function convertInstruction(
  ...
  allAccounts: ParsedMessageAccount[],
